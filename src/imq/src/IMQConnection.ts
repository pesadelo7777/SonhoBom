...
public connect() {
        ...
        // --- INÍCIO DA CORREÇÃO ---
        const connectionPromise = this.strategy.connect();

const setupStream = (stream: any) => {
            this.stream = stream;
            // Forçamos o TypeScript a tratar a stream como 'any' para evitar erros de compilação
            const streamAny = this.stream as any;

            // --- PATCH: Tradutor de Eventos ---
            if (streamAny && !streamAny.on && streamAny.addEventListener) {
                console.log('[*] WebSocket puro detetado. Traduzindo eventos...');
                streamAny.on = (event: string, callback: Function) => {
                    streamAny.addEventListener(event, (data: any) => callback(data));
                };
            }

            // --- Vinculação de Eventos ---
            if (streamAny && typeof streamAny.on === 'function') {
                streamAny.on('open', () => this.onOpen());
                streamAny.on('close', () => this.onClose());
                streamAny.on('error', (error: Error) => this.onError(error));
                streamAny.on('message', (message: any) => this.onMessage(message));
                console.log("[+] Eventos da stream vinculados com sucesso.");
            } else {
                console.error("[-] ERRO: A stream retornada não possui o método .on()!");
            }
        };

        // Trata caso a conexão seja uma Promise ou um objeto direto
        if (connectionPromise instanceof Promise) {
            connectionPromise.then(stream => setupStream(stream)).catch(err => this.onError(err));
        } else {
            setupStream(connectionPromise);
        }
        // --- FIM DA CORREÇÃO ---
    ...
    private onOpen() {
		...
		this.stream?.send(
			this.strategy.encode('msg_c2g_connect', {
                ...
                // CORREÇÃO VITAL: Usar a variável de cookie correta em vez de sessionId nulo
				cookie: this.config.cookie || this.config.sessionId,
